...
new TSession;
if(count(PCart::getItens()) > 0){

$cart = count(PCart::getItens());
...
{
    $content = file_get_contents("app/templates/{$theme}/template.html");
}
...
elseif($class == 'LoginForm'){
  AdiantiCoreApplication::loadPage('LoginForm', '', $_REQUEST);
}
else
{
    AdiantiCoreApplication::loadPage('Home', 'destaque', $_REQUEST);
}
